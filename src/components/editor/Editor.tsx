...
const Editor = (props: Props) => {
    ...
    return (
        <section id="EDITOR" className={props.isDarkMode? `${styles.themeDark} ${styles.wrapper}` : `${styles.wrapper}`} 
            aria-label='edit document'>
                <header>
                    <h2 className="text-style-heading-s">MARKDOWN</h2>
                    <button className="eyeIconButton" aria-label="enable preview mode" onClick={() => props.enablePreview()}>
                ...
                <main>
                    <textarea className="text-style-markdown-code" aria-label="edit markdown text" value={props.markdown} 
                        onInput={(e) => props.updateMarkdown(e.currentTarget.value)} />
                </main>
        </section>
    )
}
